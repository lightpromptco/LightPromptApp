...
import { Heart, Users, Calendar, MessageCircle, Plus, Settings, Star, Target } from 'lucide-react';
...
export default function SoulSyncPage() {
  ...
  const loadConnections = async () => {
    try {
      const response = await fetch(`/api/partner-connections/${user.id}`);
      if (response.ok) {
  ...
  const sendInvitation = async () => {
    if (!inviteEmail || !relationshipType) {
      toast({
    ...
    try {
      ...
      if (response.ok) {
        toast({
          ...
          description: `Invitation sent to ${inviteEmail}. They'll receive an email to join SoulSync.`
        });
        setInviteEmail('');
        setRelationshipType('');
        loadConnections();
  ...
  const addSharedGoal = async () => {
    ...
    try {
      // In a real implementation, this would be connected to a specific partnership
      const mockConnectionId = 'demo-connection';
      
      const response = await fetch(`/api/partner-connections/${mockConnectionId}/goals`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (response.ok) {
        setSharedGoals([...sharedGoals, { id: Date.now(), text: newGoal, completed: false }]);
        setNewGoal('');
        toast({
          title: 'Goal Added! 🎯',
          description: 'Your shared goal has been added to SoulSync.'
        });
      }
    } catch (error) {
      ...
      <div className="max-w-4xl mx-auto p-6 text-center">
        <Card className="p-8">
          <CardContent>
            ...
            <h2 className="text-2xl font-bold mb-4">Welcome to SoulSync</h2>
            <p className="text-muted-foreground mb-6">
              Please log in to sync your relationships and shared growth journey.
            </p>
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-8">
      ...
      <div className="text-center space-y-4">
        <div className="w-16 h-16 mx-auto bg-gradient-to-br from-pink-500 to-purple-500 rounded-2xl flex items-center justify-center">
          <Heart className="h-8 w-8 text-white" />
        </div>
        <h1 className="text-4xl font-bold">SoulSync</h1>
        <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
          Deepen your relationships through shared growth, conscious communication, and aligned intentions
        </p>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <Card>
          <CardContent className="p-6 text-center">
            ...
            <p className="text-sm text-muted-foreground">Sync Score</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <Button onClick={sendInvitation} className="w-full bg-gradient-to-r from-pink-500 to-purple-500 hover:from-pink-600 hover:to-purple-600">
            <Heart className="h-4 w-4 mr-2" />
            Send SoulSync Invitation
          </Button>
        </CardContent>
      </Card>
                ...
                <div key={connection.id} className="p-4 border rounded-lg flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground capitalize">{connection.relationshipType.replace('_', ' ')}</p>
                    </div>
                  </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Shared Goals & Intentions
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex gap-2">
            <Input
              placeholder="Add a shared goal or intention..."
              value={newGoal}
              onChange={(e) => setNewGoal(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && addSharedGoal()}
            />
            <Button onClick={addSharedGoal}>
            ...
            <div className="space-y-2">
              {sharedGoals.map((goal: any) => (
                <div key={goal.id} className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                  <input type="checkbox" checked={goal.completed} className="rounded" />
      ...
      <Card className="bg-gradient-to-br from-pink-50 to-purple-50 dark:from-pink-900/20 dark:to-purple-900/20">
        <CardContent className="p-6">
          <h3 className="text-lg font-semibold mb-3">🔗 SoulSync Integration</h3>
          <p className="text-sm text-muted-foreground mb-4">
            SoulSync shares insights with your other LightPrompt tools while respecting privacy boundaries.
          </p>
          <div className="grid md:grid-cols-2 gap-4 text-sm">
            <div>
              <h4 className="font-medium">📊 Shared with Partners:</h4>
              <ul className="text-muted-foreground mt-1 space-y-1">
                <li>• Shared goals and progress</li>
                <li>• Communication patterns</li>
                <li>• Growth milestones</li>
              </ul>
            </div>
            <div>
              <h4 className="font-medium">🔒 Kept Private:</h4>
              <ul className="text-muted-foreground mt-1 space-y-1">
                <li>• Personal wellness data</li>
                <li>• Individual chat history</li>
